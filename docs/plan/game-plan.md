...
- **Carve response**: Yaw tracks carve direction to preview the player's path; subtle carve roll tilt
- **Slope response**: Camera pitch dips on steep terrain, rises on flats (CameraSlopePitchInfluence)
- **Height offset**: TargetOffset.Z raises camera pivot to prevent terrain clipping
...
Imported static-mesh terrain plus a manually authored `APowderCoursePath` spline. `ATerrainManager` provides runtime distance/projection queries only.
...
Source/
  PowderRush/
    ...
    Terrain/        - Authored course path actor, terrain query manager, obstacle actors
...
#### B. Terrain Runtime Query System (`ATerrainManager`)
- **Level-authored runtime**: Read placed `APowderCoursePath` spline in the loaded map
- **Runtime state**: Query-only terrain manager (no runtime generation/spawn pipeline)
- **Obstacle runtime**: Obstacles are pre-placed in the map (HISM/static clutter + interactive actors)
- **Editor workflow**: Import authored static-mesh terrain, tag it `PowderTerrain`, and place/edit `APowderCoursePath`
- **No runtime procedural terrain generation** in shipping path
...
|-------|--------|---------|
| `APowderCharacter` | `Player/` | Player character, spring arm camera, snow spray |
| `UPowderMovementComponent` | `Player/` | Custom physics movement — gravity, carve, boost, ollie, airborne, freeze, tuning profiles, visual lean/pitch, additive impulse |
| `APowderPlayerController` | `Player/` | Touch + keyboard input, carve ramp, gesture tricks, double-tap ollie |
| `UPowderTrickComponent` | `Player/` | Trick system — flips, spins, spread eagle with mesh rotation |
| `IPowderAnimInterface` | `Player/` | Animation interface for future skeletal mesh characters |
| `UPowderTrickRegistry` | `Player/` | Data asset — data-driven trick definitions |
| `APowderGameMode` | `Core/` | Run lifecycle state machine, terrain/environment management |
| `UPowderGameInstance` | `Core/` | Persistent state, save/load, lifetime stats |
| `UPowderTuningProfile` | `Core/` | Data asset — movement + camera tuning with blend time |
| `UPowderSurfaceProfile` | `Core/` | Data asset — surface properties preset |
| `ATerrainManager` | `Terrain/` | Course runtime query service built from placed `APowderCoursePath` |
| `APowderCoursePath` | `Terrain/` | Canonical authored spline path for spawn/progress/respawn |
| `APowderWeatherVolume` | `Effects/` | Placed weather regions with priority + blend distance |
| `APowderTree` | `Terrain/` | Procedural tree obstacle (trunk + foliage cones) |
| `APowderRock` | `Terrain/` | Procedural rock obstacle (angular, clusters) |
| `UScoreSubsystem` | `Scoring/` | Game Instance Subsystem — scoring, combos, run stats |
| `APowderHUD` | `UI/` | Canvas HUD — menus, gameplay overlay, single-column dev tuning menu |
| `UPowderSaveGame` | `Meta/` | Save game with FLifetimeStats |
| `APowderEnvironmentSetup` | `Core/` | Spawns slope, lighting, sky, fog; owns weather manager |
| `UPowderWeatherManager` | `Effects/` | Weather transitions — lerps lighting, fog, sky, snowfall |
| `UPowderWeatherProfile` | `Effects/` | Data asset — weather configuration preset |
| `APowderSnowSpray` | `Effects/` | Niagara snow spray on carve |
| `APowderPowerup` | `Pickup/` | Speed boost + score multiplier pickups |
| `APowderJump` | `Terrain/` | Ski jump ramp — PowderTerrain-tagged, lip trigger, natural launch, ollie combo |
| `APowderFinishLine` | `Terrain/` | Finish line trigger |

### Planned Classes (Not Yet Implemented)

| Class | Module | Purpose |
|-------|--------|---------|
| `APowderAvalanche` | `Threat/` | Avalanche actor — particle wall, collision, sound |
| `UAvalancheComponent` | `Threat/` | Chase speed logic, rubberband, distance tracking |
| `USkiTrailComponent` | `Effects/` | Ribbon mesh rendering player's path in snow |
| ~~`ARampObstacle`~~ | ~~`Terrain/`~~ | ~~Replaced by `APowderJump` (already implemented)~~ |
| `AGateObstacle` | `Terrain/` | Gate pair for slalom zones |
...
| Asset | Approach | Tool |
|-------|----------|------|
| **Terrain mountain mesh** | Sculpt/retopo low-poly mountain and export as static mesh with collision LODs | Blender (free) |
...
- [x] Set up UE5 project (blank C++, mobile settings, forward renderer)
- [x] Build a single straight slope (static mesh, no generation)
- [x] Implement custom movement component with gravity-based acceleration
- [x] Implement carve mechanic (touch left/right, speed bleed, snow spray)
- [x] Implement boost meter (fills on carve, burst on release)
- [x] Implement three-quarter diorama camera (45 deg pitch, 30 deg yaw, 800-1200 arm)
- [x] Camera speed response (pitch shift, FOV widen) and carve response (yaw tracking)
- [x] Test on device (iOS) - feel must be right before proceeding

### Phase 2: The Mountain (Weeks 4-6) — Partially Complete
- [x] Build authored static-mesh mountain levels in diorama style
- [x] Query-only spline terrain service in `ATerrainManager` (no runtime spawning)
- [ ] Act-based zone weighting in `ATerrainManager` driven by distance thresholds
- [x] Obstacle collision and wipeout-as-stun (brief stun + auto-recovery, crash/respawn)
...
- [x] `APowderPowerup` — speed boost + score multiplier pickups
- [x] `APowderFinishLine` — finish line trigger
- [x] `UPowderWeatherManager` — weather transitions (lighting, fog, sky, snowfall)
- [x] Authored map pipeline: imported static-mesh terrain + placed `APowderCoursePath`
- [x] Data asset scaffolding: `UPowderSurfaceProfile`, `UPowderWeatherProfile`

### Phase 3: Tricks & Scoring (Weeks 7-9) — Partially Complete
- [x] Airborne state in `UPowderMovementComponent` (ollie launch)
- [x] `APowderJump` — natural ski jump ramp (terrain-tagged, lip trigger, ollie combo)
- [x] Swipe gesture detection in `APowderPlayerController` (up/down/left/right while airborne)
- [x] Trick state machine (`UPowderTrickComponent` — start trick, animate via mesh rotation, land/wipeout)
- [x] `UPowderTrickRegistry` — data-driven trick definitions
- [x] `IPowderAnimInterface` — animation interface for future skeletal mesh characters
- [x] Scoring integration in `UScoreSubsystem` (combo tracking, run stats)
- [ ] Near-miss detection, tree thread detection
- [x] Combo timer and multiplier system
- [x] HUD: speed, score, combo multiplier, boost meter, airborne indicator
- [x] Score screen with full breakdown
- [x] Instant restart (zero load time)
- [x] Save system (`UPowderSaveGame` with `FLifetimeStats`)
- [x] Stats screen (lifetime bests + totals)
- [x] Dev tuning menu (single-column, ~57 params, all movement/camera/surface/terrain/weather)
...
- [ ] Build remaining authored course/chunk sets (GateSlalom, MogulField, CliffRun, MixedRun)
...
| Lighting | Dynamic-near + baked-far | Crisp playable shadows near player with mobile-safe far lighting |
...
| Terrain | Imported static-mesh mountains + authored spline path | Editor-authored content only, no runtime terrain generation |
